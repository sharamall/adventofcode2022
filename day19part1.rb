...
class GeodeFinder
  ...
  def max_geodes(bp, state, day)
    if day == 24
      ...
      puts "bp: #{@bp_id} max_geo at 24 #{state[:geo]}" if state[:geo] > @max_geo
    ...
    max_possible_geodes = (24 + 1 - day) * (24 - day) / 2 # n(n-1)/2
    # max possible is new geodes if we make only geo robots today out
    # y=n(n-1)/2 crosses y = n at n = 3
    if (max_possible_geodes + state[:geo] + state[:geo_robots] * (24 - (day - 1))) < @max_geo # max possible is creating a geode at every time stamp from here on out
      res << 0
    else
      # check build robots
      if state[:ore] >= bp[:geode_cost][:ore] and state[:obs] >= bp[:geode_cost][:obs]
        ...
        # geode_rate = (24 - day) * new_state[:geo_robots] + ([bp[:geode_cost][:obs] * 1.0 / new_state[:obs_robots], bp[:geode_cost][:ore] * 1.0 / new_state[:ore_robots]].max.ceil)
...
threads.each do |t|
  t[:thread].join
  quality_sum += t[:gf].bp_id * t[:gf].max_geo
end
